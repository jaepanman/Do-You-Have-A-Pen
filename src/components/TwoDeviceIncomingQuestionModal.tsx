...
import { RoomQuestion, AnimalIdentity } from '../types';
import { SUPPLY_MAP, ANIMAL_MAP } from '../data/gameData';
import { SupplyIcon } from './SupplyIcon';
import { Volume2, Check, X, ShieldCheck, Lock, Eye, MessageCircleQuestion } from 'lucide-react';
import { speakEnglish, playYesChime, playNoTone } from '../utils/audio';

interface TwoDeviceIncomingQuestionModalProps {
  question: RoomQuestion;
  playerSecretAnimal: AnimalIdentity;
  showJapanese: boolean;
  onAnswerSupply: (hasSupply: boolean) => void;
  onAnswerGuess: (isCorrect: boolean) => void;
}

export const TwoDeviceIncomingQuestionModal: React.FC<TwoDeviceIncomingQuestionModalProps> = ({
  question,
  playerSecretAnimal,
  showJapanese,
  onAnswerSupply,
  ...
  // Supply Question
  if (question.type === 'supply' && question.supplyId) {
    const supply = SUPPLY_MAP.get(question.supplyId);
    const playerHasIt = playerSecretAnimal.supplies.includes(question.supplyId);
          ...
          <div className="bg-indigo-50 rounded-2xl p-4 border-2 border-indigo-200 text-center mb-4">
            {supply && (
              <div className="w-12 h-12 rounded-xl bg-white mx-auto flex items-center justify-center border border-indigo-200 shadow-2xs mb-2">
                <SupplyIcon id={supply.id} size="lg" />
              </div>
            )}
          ...
          <div className="bg-amber-50/90 rounded-2xl p-3.5 border-2 border-amber-300 mb-4 shadow-xs">
            ...
            <div className="grid grid-cols-2 gap-2 mb-2.5">
              {playerSecretAnimal.supplies.map((supId) => {
                const s = SUPPLY_MAP.get(supId);
                if (!s) return null;
                const isTargetMatch = supId === question.supplyId;
  ...
  // Identity Guess Question ("Are you a...?")
  if (question.type === 'identity' && question.targetAnimalId) {
    const targetAnimal = ANIMAL_MAP.get(question.targetAnimalId);
    const isActuallyCorrect = playerSecretAnimal.id === question.targetAnimalId;
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in overflow-y-auto">
        <div className="bg-white rounded-3xl p-5 sm:p-6 max-w-lg w-full shadow-2xl border-4 border-indigo-400 relative my-auto">
          ...
          <div className="bg-indigo-50 rounded-2xl p-4 border-2 border-indigo-200 text-center mb-4">
            {targetAnimal && (
              <div className="text-4xl mb-1 filter drop-shadow-xs">{targetAnimal.emoji}</div>
            )}
            <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 font-['Fredoka',sans-serif]">
              "{question.questionText}"
            </div>
